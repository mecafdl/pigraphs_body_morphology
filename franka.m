...
var_index = [q_indices,       ...      joint position
             dq_indices,      ...      joint velocity
             torque_indices,    ...    joint torque
             ang_vel_indices, ...      link angular velocty
             lin_acc_indices  ...      link linear acceleration
             ];       
...
for loop = 1:total_loops
    ...
    %dataSet   = repmat(signals(var_index,:),1,n_datasets);
