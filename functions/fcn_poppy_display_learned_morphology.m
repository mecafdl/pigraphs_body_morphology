function fcn_poppy_display_learned_morphology(lambda_hat, p_R_c, poppy, constPar, mi_graphs, parents, children, fig)
...
% Display pose
q_poppy_plot = constPar.displayConfiguration;%0*q_poppy(:, point);
...
w_T_sc(:,:,1)      = getTransform(poppy, q_poppy_plot(constPar.reference_joint_order), ...
...
for b = 1:poppy.NumBodies
    if contains(poppy.Bodies{b}.Name, 'imu')
        continue;
    end    
    fcn_highlight_bodies(poppy, b+1, [0.3 0.3 0.3],gca)
end
...
%         poppy.Bodies{constPar.imuBodyIndices(constPar.reference_body_order(c))}.Parent.Joint.Name));    
    ...
    sp_R_sc_hat(:,:,c) = p_R_c(q_poppy_plot((c-1)), gamma_hat(:,c-1));
...
%     quiver3(w_T_sc_hat(1,4,c), w_T_sc_hat(2,4,c), w_T_sc_hat(3,4,c), rotAxis(1), rotAxis(2), rotAxis(3), 1, 'k:', 'LineWidth',2); 
    ...
    jointAxis    = 0.05*gamma_hat(5:7,c-1)/norm(gamma_hat(5:7,c-1));
...
%     plot3(v(:,1),v(:,2),v(:,3),'y-','LineWidth',6)
    ...
    aux = mArrow3(v1,v2,'color','blue','stemWidth',0.002,'tipWidth',0.004,'facealpha',1);
