function fcn_phantomx_display_learned_morphology_auxiliary(lambda_hat, p_R_c, phantomx, constPar, mi_graphs, parents, children, fig)
...
% Display pose
q_poppy_plot = phantomx.homeConfiguration;%0*q_poppy(:, point);
...
w_T_sc(:,:,1)      = getTransform(phantomx, q_poppy_plot(constPar.reference_joint_order), ...
                         phantomx.Bodies{constPar.imuBodyIndices(constPar.reference_body_order(1))}.Name); 
...
% rb = show(poppy, q_poppy_plot, 'PreservePlot', false, 'Frames','off');
% alpha(rb,0.05)
% hold on    
